		...
		<div class="flex-parent-ft">
			...
			<div class="flex-child-ft item4">
				...
				<ul>
					...
					<li><a href="index.php?p=portfolio">About author</a></li>
				</ul>
			</div>
...
<script type="text/javascript">
	...
	function checkLogin(e) {
		...
		let usemailreg = /^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)*$/;
		let passwordreg = /^[A-z]{6,}\d{0,}/;

		if (!usemailreg.test(email.trim())) {
			toastr.warning("Email is not in right format");
			greske.push("Email is bad")
		}
		...
		if (greske.length == 0) {
			$.ajax({
				...
				success: function(data) {
					// console.warn(data)
					window.location.href = "index.php?p=pocetna";
				},
				error: function(xhr, status, msg) {
					console.error(xhr)
					console.error(status)
					console.error(msg)
					toastr.warning("You made a mistake, please try again!");
					br++;
					console.log(br)
					if (br % 3 == 0) {
						$.ajax({
							url: "models/users/mailer.php",
							method: "POST",
							dataType: "json",
							data: {
								email: email
							...
							success: function(data) {
								if (data) {
									toastr.success("In case you forgot your email or password, we emailed you your account information!");
								}
							},
